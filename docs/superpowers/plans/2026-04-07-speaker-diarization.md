...
- [ ] **Step 1: Добавить параметр skip_vtt в create_streaming_version**

В `app/tasks/extractor.py`, в функции `create_streaming_version` (строка 239), добавить параметр `skip_vtt=False`. В конце функции (строки 320–321) обернуть вызов `convert_srt_to_vtt` в проверку:
...
И в конце, заменить безусловный вызов:
...
- [ ] **Step 2: Создать функцию embed_subtitles_diarized**
...
git commit -m "extractor: skip_vtt в create_streaming_version, embed ASS+SRT в MKV"
```
...
async def _generate_and_embed(job: Job, file: JobFile, video_path: str, current_settings: dict):
    ...
    if job.merged_segments and job.speakers:
        ...
        if job.embed_subtitles:
            ...
            # Create streaming version (skip VTT — already generated above with colors)
